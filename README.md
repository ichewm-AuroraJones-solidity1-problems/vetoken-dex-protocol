...
| 初始铸造 | `INITIAL_MINT`，默认 `100,000,000 GOV` |
| 初始接收方 | Treasury / DAO multisig，不直接进入 Gauge |
...
| 类别 | 比例 | 锁定/释放 |
|---|---:|---|
| Liquidity Mining Reserve | 60% | 进入排放计划，由 Minter 按周释放 |
| Treasury / DAO | 20% | Timelock 管理，多签作为 proposer/canceller |
| Team / Contributors | 10% | 至少 1 年 cliff，线性释放 |
| Ecosystem / Partners | 5% | 线性释放或治理拨款 |
| Initial Liquidity / Market Making | 5% | 用于初始 GOV/WETH 等池子 |
...
| `balanceOfAt(account, timestamp)` | 查询历史快照点的用户 veGOV |
...
- 所有 lock 变更必须写入 user point 和 global point checkpoint，供 `balanceOfAt/totalSupplyAt` 查询。
...
- `balanceOfAt(account, timestamp)` 与 `totalSupplyAt(timestamp)` 用于 FeeDistributor、治理核对和历史查询，不能用当前余额替代历史余额。
...
| 投票权来源 | 使用投票生效 epoch 起点的 ve 余额 |
| 固定实现 | `VotingEscrow.balanceOfAt(user, nextEpoch)`，由已 checkpoint 的 lock slope/bias 计算，不使用分发或领取时当前余额 |
...
| 禁用 Gauge | killed gauge 不能接收新投票，下一生效周权重归零 |
...
userVe = VotingEscrow.balanceOfAt(user, nextEpoch)
...
checkpointGauge(gauge, nextEpoch, start, limit)
  -> recompute gaugeWeight[gauge][nextEpoch] from voter snapshots
  -> update totalWeight[nextEpoch] after checkpoint completes
```

checkpoint 实现规则：

- `checkpointGauge(gauge, epoch, start, limit)` 分页重算指定 Gauge 在指定 epoch 的有效权重。
- 每次重算使用 `VotingEscrow.balanceOfAt(user, epoch)` 与用户对该 Gauge 在该 epoch 生效的 bps。
...
- EmissionManager 分发某个 epoch 前，必须验证所有 active gauge 均已完成 `isCheckpointed[gauge][epoch] == true`。
- 如果存在未 checkpoint 的 active gauge，`EmissionManager.distributeMany(gauges, epoch)` 必须整体 revert，不得将未 checkpoint gauge 当作 0 权重跳过。
- 用户可调用 `poke(user, gauges[])` 主动刷新自己未来 epoch 的投票权；Keeper 仍需执行分页 checkpoint 作为最终固化。

跨周规则：

- 本周 `N` 的投票不会改变已开始的本周排放，只影响 `N + 1` 或更晚的 epoch。
- 如果用户 veGOV 在下一周快照前衰减，`checkpointGauge()` 必须按 `balanceOfAt(user, nextEpoch)` 重新计算有效投票权。
- 如果用户延长锁仓或增加锁仓，新增 ve 只影响后续投票或后续 checkpoint，不回溯修改已分发 epoch。
- `checkpointGauge` 不盲目复制上一周权重，必须按 voter 在目标 epoch 的历史 ve 余额重算。
...
| killed | 不接受新投票，下一生效周权重为 0，不获得新增 base emission |
...
| 函数 | 描述 |
|---|---|
| `addGauge(gauge, initialWeight)` | 治理添加 Gauge，可设置初始权重 |
...
- LP 退出与兑换必须使用 `amountOutMin`。
- 大额兑换前应检查 TWAP 偏离阈值。
- 只能向授权的 FeeDistributor 调用 `notifyRewardAmount(amount)`。
...
周中改锁仓只影响后续周，不回溯已结算周。若某周 `VotingEscrow.totalSupplyAt(weekTs) == 0`，该周奖励不按当前余额补分，必须滚入下一周 `weekTs + WEEK`，并发出 `ZeroVeRollover(weekTs, weekTs + WEEK, amount)`。

zero-ve rollover 执行时机：

- rollover 只能在 `notifyRewardAmount(amount)` 或 `checkpointToken()` 入账阶段执行。
- 如果 `VotingEscrow.totalSupplyAt(weekTs) == 0`，执行：
  - `tokensPerWeek[weekTs + WEEK] += tokensPerWeek[weekTs]`
  - `tokensPerWeek[weekTs] = 0`
  - emit `ZeroVeRollover(weekTs, weekTs + WEEK, amount)`
...
Pair protocol fee accounting
  ...
  -> FeeCollector calls FeeDistributor.notifyRewardAmount(amount)
...
FeeCollector -> FeeDistributor.notifyRewardAmount(amount)
```
...
6. 兑换必须使用 `amountOutMin`，大额兑换前必须检查 TWAP 偏离阈值。
7. FeeCollector 将 `rewardToken` 转入 FeeDistributor。
8. FeeCollector 调用 `FeeDistributor.notifyRewardAmount(amount)`，记录本周或当前分红周期新增奖励。
...
| 兑换保护 | 必须设置 `amountOutMin`，并检查 TWAP 偏离阈值 |
...
- `notifyRewardAmount(amount)` 会先将新增 rewardToken 归入对应周的 `tokensPerWeek[weekTs]`。
...
5. Keeper 调用 `EmissionManager.distributeMany(gauges, epoch)`，或对单个 Gauge 调用 `distributeGauge(gauge, epoch)`。
6. EmissionManager 读取已 checkpoint 的 `GaugeController.gaugeWeight(gauge, epoch)` 与 `totalWeight(epoch)`。
7. EmissionManager 计算每个 Gauge 的本周应注入 GOV。
...
else if isKilled[gauge] or gaugeWeight(gauge, epoch) == 0:
  gaugeAmount = 0
else:
  baseAmount = weeklyEmission * gaugeWeight(gauge, epoch) / totalWeight(epoch)
  repayAmount = min(rewardDebt[gauge], repayBudget * gaugeWeight(gauge, epoch) / totalWeight(epoch))
  expectedAmount = baseAmount + repayAmount
  if vaultBalance < totalExpected:
    gaugeAmount = expectedAmount * vaultBalance / totalExpected
  else:
    gaugeAmount = expectedAmount
```
...
- `sum(actualGaugeAmount) <= EmissionVault.availableBalance()`。
...
| 场景 | 处理 | 事件 |
|---|---|---|
| EmissionVault 余额不足 | 按所有 eligible Gauge 的 `expectedAmount` 比例缩放实际注入；未注入部分累积到 `rewardDebt[gauge]` | `RewardShortfall(gauge, epoch, expected, actual)` |
| 单个 Gauge 转账失败 | `distributeGauge(gauge, epoch)` 原子 revert；`distributeMany(gauges, epoch)` best-effort 记录 debt 并继续其他 Gauge | `DistributionFailed(gauge, epoch, reason)` |
| Gauge `notifyRewardAmount` 失败 | 单 Gauge 原子回滚；批量分发中回滚该 Gauge 的 Vault 转账，未注入部分进入 `rewardDebt[gauge]` | `DistributionFailed(gauge, epoch, reason)` |
...
- `distributeMany(gauges, epoch)` 是批量 best-effort 分发；单个 Gauge 失败时记录 `rewardDebt[gauge]` 和 `DistributionFailed`，继续处理其他 Gauge。
...
|---|---|---|---|---|
| `FeeDistributor.notifyRewardAmount(amount)` | rewardToken | FeeCollector | FeeDistributor | 登记协议手续费分红 |
...
| Keeper | EOA 或 bot | 无资金所有权的自动化执行者 | 调用 update/checkpoint/distribute/harvest 等周期任务 |
...
- Keeper 只负责触发公开或白名单任务，不应持有 treasury、mint、route、feeTo 等治理权限。
...
| EmissionManager | 新一轮 `distribute` | 已注入 Gauge 的用户领取 | Emergency Guardian |
...
| 添加 Gauge | `GaugeController.addGauge(address,uint256)` | Governance Timelock | `onlyOwner` |
...
| 分发到 Gauge | `EmissionManager.distributeMany(address[] gauges,uint256 epoch)` / `distributeGauge(address gauge,uint256 epoch)` | Keeper / Distributor | `onlyDistributor`，必须具备幂等保护 |
...
| I-11 | EmissionManager | `sum(actualGaugeAmount) <= EmissionVault.availableBalance()` |
...
| Foundry | 编译、测试、部署、Gas 报告 | 记录 `forge --version`，CI 与部署环境保持一致 |
| Solidity compiler | 合约编译 | `foundry.toml` 固定 `solc_version = 0.8.20` 或明确 patch 版本 |
...
阶段 1：核心基础设施
1. GovToken
2. VotingEscrow(govToken, maxLockTime)
3. PairFactory
4. Router(factory)

阶段 2：初始交易池
5. PairFactory.createPair(GOV, WETH)
6. Router.addLiquidity(...)

阶段 3：预言机
7. TwapOracle(pair_GOV_WETH, period)
8. 等待 >= PERIOD
9. TwapOracle.update()

阶段 4：手续费分红
10. FeeDistributor(rewardToken, votingEscrow)
11. FeeCollector(factory, router, rewardToken, feeDistributor)
12. PairFactory.setFeeTo(feeCollector)

阶段 5：排放激励
13. GaugeController(votingEscrow)
14. EmissionVault(govToken)
15. Minter(govToken, emissionVault)
16. EmissionManager(gaugeController, emissionVault)
17. LiquidityGauge(lpToken, govToken, votingEscrow)
18. GaugeController.addGauge(liquidityGauge, weight)
19. LiquidityGauge.setRewardDistributor(emissionManager)

阶段 6：权限移交与验证
20. GovToken grant MINTER_ROLE to Minter
21. EmissionVault authorize EmissionManager
22. 将所有 owner/admin 权限迁移到 Governance Timelock
23. 配置 DAO Multisig 为 Timelock proposer/canceller
24. 配置 Emergency Guardian 的有限暂停权限
25. 在区块浏览器验证所有合约
```
...
- GovToken 初始分配与 `INITIAL_MINT` 一致，`totalSupply <= GOV_TOKEN_CAP`。
...
| FeeCollector 最小 TWAP 检查金额 | token 配置 | 0 / token cap | Timelock | 下次 harvest 生效 | FeeCollector |
...
| 初始铸造 | 100,000,000 GOV | <= cap | 部署时配置 | 部署时 | GovToken |
...
| 最小流动性 | 1000 wei | 固定 | 部署后不可变 | 部署时 | Pair |
...
| 参数类别 | 策略 |
|---|---|
| 不变量参数 | AMM 手续费、协议费份额、最小流动性、Boost 公式、最大锁定时间等 v1 不变量部署后不可变 |
...
- GaugeController 使用 `VotingEscrow.balanceOfAt(user, nextEpoch)` 计算投票权，不使用分发或领取时当前 ve。
...
- revived gauge 不自动恢复历史权重，需要用户重新投票或治理设置新初始权重。
- `totalWeight(epoch)` 等于所有 active gauge 在该 epoch 的 `gaugeWeight` 之和。
- EmissionVault 余额不足时记录 `rewardDebt`，且不使整轮分发不可恢复。
...
- 初始分配总和必须等于 `INITIAL_MINT`，且不超过 cap。
...
- `EmissionManager.distributeMany` 必须说明 Gauge 数量对 Gas 的影响，并限制单次批量大小。
...
| 权限 | Deployer EOA 无 owner、admin、minter、distributor、notifier、guardian 权限 |
...
| FeeCollector | route、oracle、`maxTwapDeviationBps`、`minTwapCheckAmount` 已配置并 fork 测试通过 |
| Oracle | 初始 Oracle 已完成首次有效 update，`isReady == true` 且 `isStale == false` |
| FeeDistributor | notifier 指向 FeeCollector，zero-ve rollover 测试通过 |
| Emission | Minter、EmissionVault、EmissionManager 权限完整，shortfall/debt 测试通过 |
...
| 项目 | 设定 |
|---|---|
| Solidity | `^0.8.20`，基准报告需记录实际 solc patch 版本 |
| Foundry | 记录 `forge --version` 输出 |
...
- `EmissionManager.distributeMany` 必须按 Gauge 数量记录，例如 1、5、20 个 Gauge。
...
| 角色 | 责任 |
|---|---|
| Keeper Bot | 定时执行 update、checkpoint、distribute、harvest |
...
Keeper 私钥不得拥有 Timelock、Minter、Vault、route、feeTo 或 Guardian 权限。
...
| `GaugeController.checkpointGauge()` | 每周 epoch 边界后分页执行 | Keeper / anyone | epoch 开始后 6 小时未完成升级 | `script/keeper/CheckpointGauge.s.sol` |
| `Minter.updatePeriod()` | 每周 epoch 边界后 | Keeper / anyone | epoch 开始后 6 小时未完成升级 | `script/keeper/DistributeEmission.s.sol` |
| `EmissionManager.distributeMany(gauges, epoch)` | 每周 | Keeper / Distributor | epoch 开始后 12 小时未完成升级 | `script/keeper/DistributeEmission.s.sol` |
...
- Emission distribution 如为白名单执行，手动补跑者必须是 Keeper Operator 或 Governance 指定 Distributor。
